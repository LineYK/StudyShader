...
void main() {
	...
	diffCol += texture(diffuseTex, fragUV + nrm.xy).rgb * screenLight * diffAmt;

	float specAmt = max(0.0, dot(halfVec, nrm));
	float specBright = pow(specAmt, 4);
	specCol += texture(specTex, fragUV + nrm.xy).r * screenLight * specBright;
